...
class TemporalCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        w_size = numpy.prod([x * 2 - 1 for x in spatial_size])
        self.w1 = nn.Parameter(torch.zeros([w_size, feature_dim]))
        self.w2 = nn.Parameter(torch.zeros([w_size, feature_dim]))

        idx_tensor = torch.zeros([numpy.prod(spatial_size) for _ in (0, 1)], dtype=torch.long)
        for q in range(numpy.prod(spatial_size)):
            qi, qj = q // spatial_size[1], q % spatial_size[1]
            for k in range(numpy.prod(spatial_size)):
    ...
    def forward_half(self, q: torch.Tensor, k: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
        ...
        assert q.size(2) == numpy.prod(self.spatial_size)
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor):
        N, T, L, H, D = q.size()
        assert L == numpy.prod(self.spatial_size) + 1
