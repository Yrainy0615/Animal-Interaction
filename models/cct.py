...
class CrossFramelAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        a = self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
        # _, w = self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)
        # print('attention', a.shape) attention torch.Size([198, 16, 768])
        # print('weights', weights.shape) weights torch.Size([16, 198, 198])
    ...
    def forward(self, a):
        ...
        x = x.view(l+1, -1, d)
        # print(x.shape) [198,16,768]
        
        x = x + self.drop_path(self.attention(self.ln_1(x)))
        
        # x, w = self.attention(self.ln_1(x))
